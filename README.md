# 🍽️ Restaurant Website Templates - Bundle of 3

3 professional, single-file HTML restaurant website templates built for the German market. No frameworks, no dependencies - just open and edit.

---

## ⚠️ Preview Notice

These are **preview versions** for demonstration purposes only.
The full, clean versions are available for purchase on Gumroad.

👉 **[Buy the full bundle on Gumroad](https://vinayakganiger.gumroad.com/l/fhrlc)**

---

## What's Inside

| Template | Style | Target |
|---|---|---|
| 🌸 Bloom Café | Warm, cozy, terracotta | Cafés, brunch spots, bakeries |
| 🖤 Aurum Fine Dining | Dark luxury, gold accents | Upscale restaurants, wine bars |
| 🍔 SMASH Fast Casual | Bold, red & yellow, energetic | Burger joints, street food |

---

## Features (every template)

- 🇩🇪 🇬🇧 German / English language switcher (DE default)
- 🌙 Dark mode toggle
- ⚡ Animated loading screen
- 📱 Fully responsive (mobile, tablet, desktop)
- 🗺️ Google Maps embed
- 📋 Menu section with tabs
- 📸 Photo gallery
- 📝 Reservation / contact form
- 💌 Newsletter signup
- Single HTML file — zero setup

---

## Live Previews

- 🌸 [Bloom Café](./bloom-cafe.html)
- 🖤 [Aurum Fine Dining](./aurum-fine-dining.html)
- 🍔 [SMASH Fast Casual](./smash-burger.html)

---

## Built With

- Vanilla HTML, CSS, JavaScript
- Google Fonts
- Zero dependencies

---

## Contact & Custom Work

Need a customised version for your restaurant?

📧 vinayakganiger445@gmail.com
💬 [WhatsApp](https://wa.me/4916364141​97)

---

*Designed & built by **Vinayak Ganiger** · Co-created with Claude (Anthropic AI)*
